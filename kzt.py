...
def db_init():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS users (
        ...
        registered TEXT, is_premium INTEGER DEFAULT 0, last_jackpot TEXT)""")
...
def db_get(uid, username=None, is_premium=None):
    ...
    return {
        ...
        "registered": row[10], "is_premium": row[11] if len(row) > 11 else 0,
...
@dp.message(F.text.contains("Авторские"))
async def btn_author(m: types.Message):
    await m.answer(
        f"<b>👑  Авторские игры</b>\n"
        f"<i>━━━━━━━━━━━━━━━</i>\n\n"
        f"💣  <b>Мина</b>  ·  обезвредь поле\n"
        f"💎  <b>Алмаз</b>  ·  найди драгоценный камень\n"
        f"🚀  <b>Краш</b>  ·  останови ракету\n"
        f"🎡  <b>Колесо</b>  ·  крути и выигрывай",
        reply_markup=kb_author(), parse_mode="HTML"
    )
...
@dp.callback_query(F.data.in_(["mines", "diamond", "crash", "wheel"]))
async def cb_author_game(cb: types.CallbackQuery):
    uid = cb.from_user.id
    u = db_get(uid)
    game_names = {
        "mines": "💣 Мина", "diamond": "💎 Алмаз",
        "crash": "🚀 Краш", "wheel": "🎡 Колесо"
    }
    awaiting[uid] = {"game": cb.data, "bet": "main"}
    await cb.message.answer(
        f"<b>{game_names.get(cb.data, cb.data)}</b>\n\n"
        f"💰  Баланс: <b>{fmt(u['balance'])}$</b>\n\n"
...
async def msg_num(m: types.Message):
    ...
    if g == "mines":
        ...
        cells[mine_pos] = 1
        mines_state[uid] = {"cells": cells, "bet": val, "opened": [], "mult": 1.0}
        rows = []
        for j in [0, 3, 6]:
            row = []
            for i in range(j, j+3):
                row.append(InlineKeyboardButton(text="❓", callback_data=f"mine_open_{i}"))
            rows.append(row)
        kb = InlineKeyboardMarkup(inline_keyboard=rows)
        await m.answer(
            f"<b>💣  Мина</b>\n\n"
            f"💰  Ставка: <b>{fmt(val)}$</b>\n\n"
            f"<i>Открывай клетки, не попади на мину!</i>",
    ...
    if g == "wheel":
        ...
        win = seg <= 2
        mult = 3 if win else 1
        txt = f"🎡 Сектор {seg}"
        await end_game(m, uid, val, win, mult, txt, "wheel")
        return


@dp.callback_query(F.data.startswith("mine_open_"))
async def cb_mine_open(cb: types.CallbackQuery):
    uid = cb.from_user.id
    if uid not in mines_state:
        await cb.answer("Игра не найдена", show_alert=True); return
    idx = int(cb.data.replace("mine_open_", ""))
    ms = mines_state[uid]
    if idx in ms["opened"]:
        await cb.answer("Уже открыто"); return
    if ms["cells"][idx] == 1:
        u = db_get(uid)
        db_upd(uid, total_bets=round(u["total_bets"]+ms["bet"],2), games_played=u["games_played"]+1)
        del mines_state[uid]
        rows = []
        for j in [0, 3, 6]:
            row = []
        ...
        await cb.message.edit_text(
            f"<b>💥  БУМ!</b>\n\n"
            f"💸  Потеряно: <b>-{fmt(ms['bet'])}$</b>\n"
            f"💰  Баланс: <b>{fmt(u['balance'])}$</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
            parse_mode="HTML"
        )
        await cb.answer("💥 Мина!", show_alert=True); return
    ms["opened"].append(idx)
    ms["mult"] = round(ms["mult"] * 1.4, 2)
    rows = []
    for j in [0, 3, 6]:
        row = []
        for i in range(j, j+3):
            t = "💎" if i in ms["opened"] else "❓"
            row.append(InlineKeyboardButton(text=t, callback_data=f"mine_open_{i}"))
        rows.append(row)
    rows.append([InlineKeyboardButton(text=f"💰 Забрать x{ms['mult']}", callback_data="mine_cash")])
    try: await cb.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    except: pass
    await cb.answer(f"💎 x{ms['mult']}")


@dp.callback_query(F.data == "mine_cash")
async def cb_mine_cash(cb: types.CallbackQuery):
    uid = cb.from_user.id
    if uid not in mines_state:
        await cb.answer("Игра не найдена", show_alert=True); return
    ms = mines_state[uid]
    win_amount = round(ms["bet"] * ms["mult"], 2)
    u = db_get(uid)
    nb = round(u["balance"] + win_amount, 2)
    db_upd(uid, balance=nb, total_bets=round(u["total_bets"]+ms["bet"],2),
           games_played=u["games_played"]+1, total_won=round(u["total_won"]+win_amount,2))
    del mines_state[uid]
    await cb.message.edit_text(
        f"<b>✅  Забрал выигрыш!</b>\n\n"
        f"🎯  x{ms['mult']}\n"
        f"➕  Выигрыш: <b>+{fmt(win_amount)}$</b>\n"
        f"💰  Баланс: <b>{fmt(nb)}$</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔄 Снова", callback_data="mines"),
             InlineKeyboardButton(text="🏠 В меню", callback_data="menu_games")]
        ]),
        parse_mode="HTML"
    )
    await cb.answer(f"💰 +{fmt(win_amount)}$")
...
async def end_game(msg, uid, bet, win, mult, text_res, game_code, dice_msg=None):
    ...
    if bet >= BIG_BET and CHANNEL_ID:
        try:
            ...
            if dice_msg:
                try: await dice_msg.forward(chat_id=CHANNEL_ID)
                except: pass
        except Exception as e:
...
async def cb_cp_amt(cb: types.CallbackQuery):
    ...
    if d == "custom":
        ...
        await cb.message.answer("✏  Введи сумму (1-10000$):")
        await cb.answer(); return
...
async def cb_ton_amt(cb: types.CallbackQuery):
    ...
    if d == "custom":
        ...
        await cb.message.answer("✏  Введи сумму в TON (0.1-1000):")
        await cb.answer(); return
...
async def do_ton_deposit(msg, uid, ton_amount):
    if not TON_WALLET:
        await msg.answer("❌  TON_WALLET не настроен"); return
    ...
    await msg.answer(
        ...
        f"⚠️  <b>Комментарий:</b> <code>RT{uid}</code>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_toncheck(cb: types.CallbackQuery):
    ...
    await cb.message.answer(f"<b>⏳  Проверка платежа...</b>", parse_mode="HTML")
    usd_amount = round(ton_amount * TON_USD, 2)
    try:
        await bot.send_message(ADMIN_ID,
            f"<b>💠  TON платёж</b>\n\n👤 <code>{uid}</code>\n💵 {ton_amount} TON ({usd_amount}$)\n💬 RT{uid}\n\nЗачислить: <code>/addton {uid} {ton_amount}</code>",
            parse_mode="HTML")
    except: pass
    await cb.answer()
...
async def cmd_addton(m: types.Message):
    if m.from_user.id != ADMIN_ID: return
    args = m.text.split()
    if len(args) < 3: await m.answer("📋 /addton <uid> <ton>"); return
    try:
        uid = int(args[1]); ton_amount = float(args[2])
    ...
    db_upd(uid, balance=nb, total_deposit=nd)
    await m.answer(f"<b>✅  Зачислено</b>\n\n👤 <code>{uid}</code>\n💠 {ton_amount} TON = {usd}$\n💰 {fmt(nb)}$", parse_mode="HTML")
    try: await bot.send_message(uid, f"<b>✅  TON пополнение!</b>\n\n💠 {ton_amount} TON\n💰 +{usd}$", parse_mode="HTML")
    except: pass
...
async def cb_st_amt(cb: types.CallbackQuery):
    ...
    if d == "custom":
        ...
        await cb.message.answer("✏  Введи кол-во Stars (50-10000):")
        await cb.answer(); return
...
async def on_stars_paid(m: types.Message):
    ...
    try:
        ...
        nd = round(u["total_deposit"] + usd, 2)
        db_upd(uid, balance=nb, total_deposit=nd)
        await m.answer(f"<b>✅  Оплата Stars успешна!</b>\n\n⭐ {stars} Stars\n💰 +{usd}$\n💵 Баланс: <b>{fmt(nb)}$</b>", parse_mode="HTML")
        if CHANNEL_ID:
            try: await bot.send_message(CHANNEL_ID, f"<b>💰  НОВОЕ ПОПОЛНЕНИЕ!</b>\n\n👤 <b>{u['username']}</b>\n⭐ {stars} Stars ({fmt(usd)}$)", parse_mode="HTML")
            except: pass
    except Exception as e:
...
async def cb_ad_stats(cb: types.CallbackQuery):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*), SUM(balance), SUM(total_bets), SUM(games_played), SUM(total_deposit), SUM(refs), SUM(is_premium) FROM users")
    r = c.fetchone()
    conn.close()
    await cb.message.edit_text(
        f"<b>📊  Статистика</b>\n"
        ...
        f"⭐  Premium: <b>{r[6] or 0}</b>\n"
        f"💰  Баланс: <b>{fmt(r[1] or 0)}$</b>\n"
...
async def cb_ad_users(cb: types.CallbackQuery):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT username, balance, refs, is_premium FROM users ORDER BY balance DESC LIMIT 30")
    rows = c.fetchall()
    conn.close()
    text = "<b>👥  Все игроки</b>\n<i>━━━━━━━━━━━━━━━</i>\n\n"
    for i, (n, b, r, p) in enumerate(rows, 1):
        star = "⭐ " if p else ""
        text += f"{i}. {star}<b>{n}</b> — {fmt(b)}$ | {r} реф\n"
...
async def cb_ad_multicheck(cb: types.CallbackQuery):
    ...
    await cb.message.answer(
        f"<b>🎫  МультиЧек</b>\n\n"
        f"<b>Команда:</b>\n"
        f"<code>/multicheck 100 10 5</code>",
        parse_mode="HTML"
    )
    await cb.answer()
...
async def cmd_multicheck(m: types.Message):
    ...
    if CHANNEL_ID:
        try:
            ...
            await m.answer(f"<b>✅  МультиЧек создан!</b>\n\n🎫 Код: <code>{code}</code>", parse_mode="HTML")
        except Exception as e:
...
async def cb_mc_claim(cb: types.CallbackQuery):
    ...
    uid = cb.from_user.id
    prem = is_premium_user(cb.from_user)
    username = uname(cb.from_user)
    u = db_get(uid, username, is_premium=prem)
    ...
    if u["total_bets"] < min_turnover:
        await cb.answer(
            f"❌ Недостаточный оборот!\n\nТребуется: {fmt(min_turnover)}$\nУ вас: {fmt(u['total_bets'])}$",
            show_alert=True
        )
        return
...
async def cmd_bonuscheck(m: types.Message):
    ...
    await m.answer(f"<b>✅  Бонус выдан</b>\n\n👤  {u['username']}\n➕  +{amt}$\n💰  {fmt(nb)}$", parse_mode="HTML")
    try:
        await bot.send_message(tid, f"<b>🎁  Вам выдан бонус!</b>\n\n➕  +{amt}$\n💰  {fmt(nb)}$", parse_mode="HTML")
    except: pass
    ...
    while True:
        try:
            ...
            if uids:
                ...
                for uid in uids:
                    try:
                        await bot.send_message(uid, msg_text, reply_markup=kb, parse_mode="HTML")
                        sent += 1
...
async def main():
    ...
    print("📢 Авто-рассылка запущена (каждые 1 час)")
