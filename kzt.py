...
async def cmd_multicheck(m: types.Message):
    ...
    if CHANNEL_ID:
        try:
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="Активировать чек", callback_data=f"mc_claim_{code}", icon_custom_emoji_id=PE_CARD)]
            ])
            await bot.send_message(
                CHANNEL_ID,
                f"<b>🎫  МУЛЬТИЧЕК</b>\n"
    ...
    while True:
        try:
            ...
            if uids:
                ...
                kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="Играть сейчас", callback_data="menu_games", icon_custom_emoji_id=PE_FIRE)],
                ])
                sent = 0
                failed = 0
                for uid in uids:
